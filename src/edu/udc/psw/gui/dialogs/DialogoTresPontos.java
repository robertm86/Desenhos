...
public class DialogoTresPontos extends JDialog {
	...
	public static void main(String[] args) {
		try {
			DialogoTresPontos dialog = new DialogoTresPontos("Ler tres pontos");
	 ...
	 * Create the dialog.
	 */
	public DialogoTresPontos(String titulo) {
		...
		contentPanel.setLayout(layout);
			...
			JLabel lblX3 = new JLabel("Coordenada x3:");
			layout.putConstraint(SpringLayout.NORTH, lblX3, 6, SpringLayout.SOUTH, lblY2);
			layout.putConstraint(SpringLayout.WEST, lblX3, 11, SpringLayout.WEST, contentPanel);
			contentPanel.add(lblX3);

			JLabel lblY3 = new JLabel("Coordenada y3:");
			layout.putConstraint(SpringLayout.NORTH, lblY3, 6, SpringLayout.SOUTH, lblX3);
			layout.putConstraint(SpringLayout.WEST, lblY3, 11, SpringLayout.WEST, contentPanel);
			contentPanel.add(lblY3);
			...
			layout.putConstraint(SpringLayout.NORTH, textX3, 6, SpringLayout.SOUTH, textY2);
			layout.putConstraint(SpringLayout.NORTH, lblX3, 3, SpringLayout.NORTH, textX3);
			layout.putConstraint(SpringLayout.WEST, textX3, 100, SpringLayout.WEST, contentPanel);
			layout.putConstraint(SpringLayout.EAST, textX3, 200, SpringLayout.WEST, contentPanel);
			textX3.setEnabled(true);
			textX3.setEditable(true);
			textX3.setText("");
			contentPanel.add(textX3);
			textX3.setColumns(10);
			
			textY3 = new JTextField();
			layout.putConstraint(SpringLayout.NORTH, textY3, 6, SpringLayout.SOUTH, textX3);
			layout.putConstraint(SpringLayout.NORTH, lblY3, 3, SpringLayout.NORTH, textY3);
			layout.putConstraint(SpringLayout.WEST, textY3, 100, SpringLayout.WEST, contentPanel);
			layout.putConstraint(SpringLayout.EAST, textY3, 200, SpringLayout.WEST, contentPanel);
			textY3.setEnabled(true);
			textY3.setEditable(true);
			textY3.setText("");
			contentPanel.add(textY3);
	...
	private boolean valida()
	{
		...
		if(textY2.getText() == null || textY2.getText().length() == 0)
			return false;
		if(textX3.getText() == null || textX3.getText().length() == 0)
			return false;
		if(textY3.getText() == null || textY3.getText().length() == 0)
			return false;
		...
		if(!textY2.getText().matches("[0-9]+"))
			return false;
		if(!textX3.getText().matches("[0-9]+"))
			return false;
		if(!textY3.getText().matches("[0-9]+"))
			return false;
		return true;
	}
	...
	{
		if(result == OK)
			return Integer.parseInt(textX3.getText());
		else
			return 0;
	}
	public int getY3()
	{
		if(result == OK)
			return Integer.parseInt(textY3.getText());
		else
			return 0;
	}
